describe("Test the skillGroup POST module", () => {
  test("The skillGroup POST module can be required via the index", async () => {
    // GIVEN the module
    ...
    const skillGroupPOSTModule = await import("./");

    // THEN check if Schemas is defined in it
    expect(skillGroupPOSTModule.default.Schemas.Response.Payload).toBeDefined();
    expect(skillGroupPOSTModule.default.Schemas.Request.Payload).toBeDefined();

    // AND check if constants are defined in it
    const Constants = skillGroupPOSTModule.default.Constants;
  ...
  test("The export module matches the snapshot", () => {
    ...
    expect(async () => {
      ...
      const skillGroupPOSTModule = await import("./");
      expect(skillGroupPOSTModule.default).toMatchSnapshot();
    }).not.toThrowError();
  });
});
