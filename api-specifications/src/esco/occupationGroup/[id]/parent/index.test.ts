describe("Test the occupationGroup parent module", () => {
  ...
  test("The occupationGroup POST parent module can be required via the index", async () => {
    // GIVEN the module
    // WHEN the module is required via the index
    expect(async () => await import("./")).not.toThrow(); // We check that it doesn't throw an error instead of simply letting it fail on import because we want an easier error message
    const occupationGroupPOSTParentModule = await import("./");

    // THEN check if Schemas is defined in it
    expect(occupationGroupPOSTParentModule.default.POST.Schemas.Request.Payload).toBeDefined();
    expect(occupationGroupPOSTParentModule.default.POST.Schemas.Response.Payload).toBeDefined();
  });
