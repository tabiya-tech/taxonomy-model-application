...
describe("Test the Error Schema", () => {
  ...
  testValidSchema("APIError.Schemas.POST.Payload", APIError.Schemas.POST.Payload);
});

describe("Validate JSON against the APIError POST Schema", () => {
  // GIVEN a valid APIError POST object
  const givenValidAPIError: APIError.Types.POST = {
  ...
  testSchemaWithValidObject("APIError.Schemas.POST.Payload", APIError.Schemas.POST.Payload, givenValidAPIError);

  // AND WHEN the object has additional properties
  // THEN expect the object to not validate
  testSchemaWithAdditionalProperties(
    "APIError.Schemas.POST.Payload",
    APIError.Schemas.POST.Payload,
    givenValidAPIError
  );
  ...
  describe("Validate APIError.Schemas.Payload fields", () => {
    describe("Test validate of 'errorCode'", () => {
      const allValidErrorCodes = [
        ...
        ...Object.values(SkillGroupAPI.POST.Enums.Response.Status400.ErrorCodes),
        ...Object.values(SkillGroupAPI.POST.Enums.Response.Status404.ErrorCodes),
        ...Object.values(SkillGroupAPI.POST.Enums.Response.Status500.ErrorCodes),
      ...
      test.each([
        ...
        const givenObject: APIError.Types.POST = {
          //@ts-ignore
          errorCode: givenValue,
        };
        // THEN expect the object to validate accordingly
        //@ts-ignore
        assertCaseForProperty("errorCode", givenObject, APIError.Schemas.POST.Payload, caseType, failureMessages);
      });
    });

    describe("Test validate of 'message'", () => {
      testStringField<APIError.Types.POST>("message", ErrorConstants.MAX_MESSAGE_LENGTH, APIError.Schemas.POST.Payload);
    });

    describe("Test validate of 'details'", () => {
      testStringField<APIError.Types.POST>("details", ErrorConstants.MAX_DETAILS_LENGTH, APIError.Schemas.POST.Payload);
    });
  });
});
