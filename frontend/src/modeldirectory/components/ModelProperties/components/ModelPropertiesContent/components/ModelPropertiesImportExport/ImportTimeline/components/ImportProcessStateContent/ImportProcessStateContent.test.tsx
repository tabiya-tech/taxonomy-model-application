...
describe("ImportProcessStateContent", () => {
  ...
  test("should render properly when the import process state doesnt provide a createdAt date", () => {
    // GIVEN an import process state without a createdAt date
    const givenImportProcessState = {
      id: "000000000000000000000001",
      result: {
        errored: true,
    ...
    expect(DurationPropertyField).toHaveBeenCalledWith(
      {
        label: "Duration",
        firstDate: expect.any(Date), // we pass the current date
        secondDate: undefined,
        fieldId: FIELD_ID.IMPORT_DURATION_FIELD,
        "data-testid": DATA_TEST_ID.IMPORT_DURATION_FIELD,
      },
