...
describe("FileEntry render tests", () => {
  ...
  it.each([
    ...Object.values(ImportAPISpecs.Constants.ImportFileTypes).map(
      (fileType) =>
        [fileType as ImportAPISpecs.Constants.ImportFileTypes, mapFileTypeToName(fileType)] as [
          ImportAPISpecs.Constants.ImportFileTypes,
          string,
        ]
    ),
  ])(
    "should render default state %s -> %s",
    (fileType: ImportAPISpecs.Constants.ImportFileTypes, expectedFileTypeName: string) => {
