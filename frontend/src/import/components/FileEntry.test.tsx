...
describe("FileEntry render tests", () => {
  ...
  it.each(
    [...Object.values(Import.Types.ImportFileTypes).map((fileType) => [fileType as Import.Types.ImportFileTypes, mapFileTypeToName(fileType)] as [Import.Types.ImportFileTypes, string])]
  )
  ("should render default state %s -> %s", (fileType: Import.Types.ImportFileTypes, expectedFileTypeName: string) => {
  ...
  it("multiple components should have a unique id", () => {
    // GIVEN some filetype
    const givenFileType = Import.Types.ImportFileTypes.ESCO_OCCUPATION;
