...
export interface ModelLocaleSelectProps {
  locales: Locale.Types.ILocale[],
  notifyModelLocaleChanged?: (locale: Locale.Types.ILocale) => any
}
...
const ModelLocalSelectField = (props: ModelLocaleSelectProps) => {
  ...
  return <FormControl sx={{width: '100%'}} data-testid={DATA_TEST_ID.MODEL_LOCALE_SELECT_FIELD}>
    <Stack className={classes.fieldStack} spacing={0.5}>
      ...
      >
        {props.locales?.map((locale) =>
          <MenuItem
            data-testid={DATA_TEST_ID.MODEL_LOCALE_ITEM}
            key={locale.UUID}
            value={locale.UUID}
          >
