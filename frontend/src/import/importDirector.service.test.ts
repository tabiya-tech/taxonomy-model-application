...
describe("Test the import director service", () => {
  ...
  test.each([
    ["the languages declared in the model info file", ["en", "fr"], ["en", "fr"]],
    [
      "the fall back language of the registry when the model info file declares none",
      [],
      [LanguageAPISpecs.Constants.FALLBACK_LANGUAGE.shortCode],
    ],
  ])(
    "should successfully direct the import and create the model with %s",
    async (_description, givenAvailableLanguages, expectedAvailableLanguages) => {
      ...
      // AND the model service is called with the given arguments (name, description, locale) and the expected languages
      expect(modelService.createModel).toHaveBeenCalledWith({
        ...
        availableLanguages: expectedAvailableLanguages,
      });
