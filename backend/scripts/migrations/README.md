...
| `0003-occupation-groups-localized-fields` | Rewrites `preferredLabel`, `description` and `altLabels` of every occupation group into localized sub documents keyed by the fall back language, e.g. `"x"` into `{ "en": "x" }`. `down` flattens them back to the value of the fall back language. `code` and `groupType` are monolingual and are left untouched. |
