...
import { initializeSchemaAndModel } from "../../src/esco/skill/model/skill.model";
...
import { TranslatedStringKey } from "../../src/common/language/translatedString.types";
import { MongooseModelName } from "../../src/esco/common/mongooseModelNames";
import { ISkillDoc } from "../../src/esco/skill/_shared/skill.types";
import { IMigration, IMigrationResult } from "./migration.types";

/**
 * Rewrites the Skill collection's translatable fields between a plain string (or array, for altLabels) and a
 * localized sub document keyed by the fallback language, e.g. "Cook" <-> { en: "Cook" }.
 */

const BATCH_SIZE = 500;
const STRING_FIELDS = ["preferredLabel", "description", "definition", "scopeNote"];

// reuses the model if the connection already has it registered, since mongoose refuses to register one twice
function getSkillCollection(connection: mongoose.Connection): mongoose.Collection {
  const model =
    (connection.models[MongooseModelName.Skill] as mongoose.Model<ISkillDoc>) ?? initializeSchemaAndModel(connection);
...
function isLocalizedValue(value: unknown): value is Partial<Record<TranslatedStringKey, string>> {
...
function buildRevertUpdate(doc: Record<string, unknown>, fallbackDbKeyName: string): Record<string, unknown> | null {
  ...
  if (Array.isArray(doc.altLabels) && doc.altLabels.some(isLocalizedValue)) {
    update.altLabels = (doc.altLabels as unknown[]).map((item) =>
...
async function migrateInBatches(
  ...
  const collection = getSkillCollection(connection);
  let matched = 0;
  let modified = 0;
...
export async function migrateSkillsLocalizedFields(connection: mongoose.Connection): Promise<IMigrationResult> {
  return migrateInBatches(
    connection,
    { preferredLabel: 1, description: 1, definition: 1, scopeNote: 1, altLabels: 1 },
    buildMigrationUpdate,
    "Skills localized fields migration"
  );
}

const migration: IMigration = {
  name: "0004-skills-localized-fields",
  description:
    "Rewrites the Skill collection's translatable fields into localized sub documents keyed by the fallback " +
  ...
  async up(connection: mongoose.Connection): Promise<IMigrationResult> {
    const result = await migrateSkillsLocalizedFields(connection);
    console.info(`Localized the translatable fields of ${result.modified} skill(s).`);
    return result;
  },
  ...
  async down(connection: mongoose.Connection): Promise<IMigrationResult> {
    ...
    const result = await migrateInBatches(
      ...
      "Skills localized fields migration (down)"
    );
    console.info(`Flattened the translatable fields of ${result.modified} skill(s).`);
    return result;
  },
};
