...
import { AssociationType, Differences, DifferencesValue, EntityType } from "./types";
...
function compareEntities<T extends Entity>(
  ...
  compareEntitiesFn: (leftEntity: T, rightEntity: T) => DifferencesValue,
...
function compareRelations<T extends Relation>(
  ...
  compareRelationshipsFn: (leftRelationship: T, rightRelationship: T) => DifferencesValue,
  differences: Differences
) {
  if (!leftModelManager || !rightModelManager) {
  ...
  for (const leftRelation of leftRelations) {
    try {
      ...
      const diffs: DifferencesValue = compareRelationshipsFn(leftRelation, rightRelation);
...
export function findDifferences(leftModelManager: IModelManager, rightModelManager: IModelManager) {
  ...
  const differences: Differences = {
    models: {
      left: leftModelManager.state.getModelName(),
      right: rightModelManager.state.getModelName(),
    },
    left: {
      entities: [],
      associations: [],
