...
function addUUID(sourceFolder: string, file: string, targetFolder: string) {
...
function createBaseModel({ source, destination }: { source: string; destination: string }) {
  ...
  addUUID(source, "model_info.csv", destination);
