...
/**
 * The language registry is a bundled constant of api-specifications, the backend consumes it with a plain import from
 * the linked package. These tests guard that consumption: that the "api-specifications/language" subpath resolves,
 * that the registry the backend sees is the bundled one, and that it is not mutable from here.
 */
describe("Test the language registry as it is consumed by the backend", () => {
  ...
  test("the language registry should contain at least english and french", () => {
    ...
    // AND the short codes the backend expects to be supported at a minimum
  ...
  test("the language registry should not be mutable by the backend", () => {
    ...
    // WHEN the backend tries to add the language to the registry
