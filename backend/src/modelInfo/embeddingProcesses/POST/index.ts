...
export class POSTModelEmbeddingProcessesHandler {
  ...
  async handle(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    const validationResult =
      validateEvent<ModelInfoApiSpecs.ModelInfo.EmbeddingProcessStates.POST.Types.Request.Payload>(
        event,
        ModelInfoApiSpecs.ModelInfo.EmbeddingProcessStates.POST.Schemas.Request.Payload,
        ModelInfoApiSpecs.ModelInfo.EmbeddingProcessStates.POST.Constants.MAX_PAYLOAD_LENGTH
      );
    ...
    try {
      ...
      if (error instanceof ModelNotFoundError) {
        return errorResponse(
          ...
          ModelInfoApiSpecs.ModelInfo.EmbeddingProcessStates.POST.Enums.Response.Status404.ErrorCodes
