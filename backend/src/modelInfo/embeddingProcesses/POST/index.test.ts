...
describe("Test the POSTModelEmbeddingProcessesHandler", () => {
  ...
  test("should respond with NOT_FOUND when the model does not exist", async () => {
    ...
    // THEN expect a 404 response with the NOT_FOUND error code
    expect(actualResponse.statusCode).toEqual(StatusCodes.NOT_FOUND);
    expect(JSON.parse(actualResponse.body).errorCode).toEqual(ErrorAPISpecs.Constants.ErrorCodes.NOT_FOUND);
  });
