...
export const handleRouteEvent = async (event: APIGatewayProxyEvent) => {
  ...
  if (path === Routes.APPLICATION_INFO_ROUTE) {
  ...
  } else if (pathToRegexp([Routes.MODEL_ROUTE]).regexp.test(path)) {
    return ModelInstanceHandler(event);
  } else if (
    pathToRegexp([
    ...
    pathToRegexp([
      ...
      Routes.OCCUPATION_HISTORY_ROUTE,
    ]).regexp.test(path)
  ) {
    return OccupationHandler(event);
  } else if (
    pathToRegexp([
      Routes.SKILL_GROUPS_ROUTE,
