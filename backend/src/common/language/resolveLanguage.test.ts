...
const FALL_BACK_SHORT_CODE = LanguageAPISpecs.Constants.FALL_BACK_LANGUAGE.shortCode;
...
describe("Test resolveLanguageConfig()", () => {
  ...
  test("should return the configuration of the fall back language when the client and the model agree on no language", () => {
    ...
    expect(actualLanguage).toBe(LanguageAPISpecs.Constants.FALL_BACK_LANGUAGE);
  });
});
