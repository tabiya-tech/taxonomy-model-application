  ...
  _transform(
    ...
    try {
      ...
      try {
        ...
        const error = new Error(`Failed to transform SkillHierarchy to CSV row: ${json}`, { cause: e });
        console.error(error);
        callback(error);
      }
    }
...
const SkillHierarchyToCSVTransform = (modelId: string): Readable => {
  ...
  return pipeline(
    ...
    (error) => {
      if (error) {
        console.error(new Error("Transforming SkillHierarchy to CSV failed", { cause: error }));
      }
    }
  );
};
