  ...
  _transform(
    ...
    try {
      ...
      try {
        ...
        const error = new Error(`Failed to transform SkillGroup to CSV row: ${json}`, { cause: e });
        console.error(error);
        callback(error);
      }
    }
...
const SkillGroupsToCSVTransform = (modelId: string): Readable => {
  ...
  return pipeline(
    ...
    (error) => {
      if (error) {
        console.error(new Error("Transforming SkillGroups to CSV failed", { cause: error }));
      }
    }
  );
};
