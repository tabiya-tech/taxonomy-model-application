...
describe('getStdHeadersValidator', () => {
  ...
  test('should return false when actual headers do not include all expected headers', async () => {
    ...
    expect(importLogger.logError).toHaveBeenCalledWith(`Failed to validate header for ${givenValidatorName}, expected to include header header2`);
    expect(importLogger.logError).toHaveBeenCalledWith(`Failed to validate header for ${givenValidatorName}, expected to include header header3`);
  });
});
