    ...
    async (
      ...
      const mockRepository: IOccupationRepository = {
        ...
        createManyLocalized: jest
          .fn()
          .mockImplementation((specs: INewOccupationSpecLocalized[]): Promise<IOccupation[]> => {
            return Promise.resolve(
              specs.map((spec: INewOccupationSpecLocalized): IOccupation => {
                return {
                  ...spec,
      ...
      const expectedSpecs: INewOccupationSpecLocalized[] = expectedFileModule.expected.map(
        (expectedSpec: Omit<INewOccupationSpecLocalized, "modelId">) => {
          return { ...expectedSpec, modelId: givenModelId };
        }
      );

      expect((mockRepository.createManyLocalized as jest.Mock).mock.lastCall[0]).toIncludeSameMembers(expectedSpecs);
      ...
      expectedSpecs
        .filter((res: Omit<INewOccupationSpecLocalized, "modelId">) => isSpecified(res.importId))
        .forEach((expectedSpec: Omit<INewOccupationSpecLocalized, "modelId">, index: number) => {
      ...
      expect(errorLogger.logWarning).toHaveBeenNthCalledWith(
        4,
        "Warning while importing Occupation row with id:'key_2'. Preferred label (en) 'preferred\n" +
          "label\n" +
          "with\n" +
      ...
      expect(errorLogger.logWarning).toHaveBeenNthCalledWith(
        7,
        "Warning while importing ESCO Occupation row with id:'key_3'. AltLabels (en) contain 1 duplicates."
      );
      expect(errorLogger.logWarning).toHaveBeenNthCalledWith(
        11,
        "Warning while importing Local Occupation row with id:'key_4'. AltLabels (en) contain 1 duplicates."
      );
    }
  );
});
