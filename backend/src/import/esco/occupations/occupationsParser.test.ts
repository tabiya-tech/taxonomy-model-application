...
describe("test parseOccupations from", () => {
  ...
  test.each([
    ["url file", "./src/import/esco/occupations/_test_data_/given.csv", parseFromUrlCallback],
    ["csv file", "./src/import/esco/occupations/_test_data_/given.csv", parseFromFileCallback]
  ])
  ("should create Occupations from %s for rows with importId", async (description, file, parseCallBack: (file: string, givenModelId: string, importIdToDBIdMap: Map<string, string>) => Promise<RowsProcessedStats>) => {
    // GIVEN a model id
    ...
    expect(importLogger.logWarning).toHaveBeenNthCalledWith(1, "Failed to import Occupation from row:1 with importId:");
    expect(importLogger.logWarning).toHaveBeenNthCalledWith(2, "Failed to import Occupation from row:2 with importId:");
  });
});