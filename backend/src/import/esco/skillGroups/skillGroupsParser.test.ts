...
describe("test parseSkillGroups from", () => {
  ...
  ("should create SkillGroups from %s", async (description, file, parseCallBack: (file: string, givenModelId: string, importIdToDBIdMap: Map<string, string>) => Promise<RowsProcessedStats>) => {
    // GIVEN a model id
    ...
    expect(importLogger.logWarning).toHaveBeenNthCalledWith(1, "Failed to import SkillGroup from row:1 with importId:");
    expect(importLogger.logWarning).toHaveBeenNthCalledWith(2, "Failed to import SkillGroup from row:2 with importId:");
  })
});