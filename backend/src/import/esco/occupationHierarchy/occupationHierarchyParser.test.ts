...
describe("test parseOccupationHierarchy from", () => {
  ...
  ("should create Occupation Hierarchy from %s for valid rows", async (description, file, parseCallBack: (file: string, givenModelId: string, importIdToDBIdMap: Map<string, string>) => Promise<RowsProcessedStats>) => {
    // GIVEN a model id
    ...
    jest.spyOn(givenImportIdToDBIdMap, "get").mockImplementation((key) => {
      if(key === "") // empty string is not a valid import id and should be ignored to enable the test how they are handled
        return undefined;
      else
        return "mapped_" + key;
    })
    ...
    expect(importLogger.logWarning).toHaveBeenNthCalledWith(1, `Failed to import OccupationHierarchy row with parentType:'invalid should be ignored' and childType:'EscoOccupation'`);
    expect(importLogger.logWarning).toHaveBeenNthCalledWith(2, `Failed to import OccupationHierarchy row with parentType:'ISCOGroup' and childType:'invalid should be ignored'`);
    expect(importLogger.logWarning).toHaveBeenNthCalledWith(3, `Failed to import OccupationHierarchy row with parent importId:'' and child importId:'key_11'`);
    expect(importLogger.logWarning).toHaveBeenNthCalledWith(4, `Failed to import OccupationHierarchy row with parent importId:'key_12' and child importId:''`);
  });
});