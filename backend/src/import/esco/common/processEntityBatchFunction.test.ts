...
describe("test getProcessEntityBatchFunction", () => {
  ...
  test("should create entities from for rows with importId", async () => {
    ...
    const givenMockRepository = {
      // @ts-ignore
      createMany: jest.fn().mockImplementation((specs: GivenNewEntitySpec[]): Promise<GivenEntity[]> => {
        return Promise.resolve(specs.map((spec: GivenNewEntitySpec): GivenEntity => {
          return {
            ...spec,
    ...
    const processEntityBatchFunction = getProcessEntityBatchFunction<GivenEntity, GivenNewEntitySpec>(
      givenEntityName,
      givenMockRepository,
  ...
  test("should create entities from for rows with importId and log warnings for rows without importId", async () => {
    ...
    expect(givenMockRepository.createMany).toHaveBeenCalledWith(
      [givenBatch[0], givenBatch[2], givenBatch[4], givenBatch[6]]
    )
    ...
    [givenBatch[1], givenBatch[3], givenBatch[5], givenBatch[7]].forEach((givenSpec, index: number) => {
      expect(importLogger.logWarning).toHaveBeenNthCalledWith(index + 1, expect.stringContaining(`Failed to import ${givenEntityName} from row:${2 * (index + 1)} with importId:`));
    });
  });
  ...
  test("should log warnings with correct row number", async () => {
    ...
    givenSecondBatch.forEach((givenSpec, index: number) => {
      expect(importLogger.logWarning).toHaveBeenNthCalledWith(index + 1, expect.stringContaining(`Failed to import ${givenEntityName} from row:${givenFirstBatch.length + index + 1} with importId:`));
    });
  });
...
// AND a map to map the ids of the CSV file to the database ids
    ...
    expect(importLogger.logError).toHaveBeenCalledWith(expect.stringContaining(`Failed to process ${givenEntityName}s batch`), givenError);
    // AND a warning should be logged for each row
    for (let i = 1; i <= givenBatch.length; i++) {
      expect(importLogger.logWarning).toHaveBeenNthCalledWith(i, expect.stringContaining(`Failed to import ${givenEntityName} from row:${i} with importId:${i}`));
    }
  });
  ...
  test("should log warnings when repository does create some of the batch entries", async () => {
    ...
    const givenMockRepository = {
      // @ts-ignore
      createMany: jest.fn().mockImplementation((specs: GivenNewEntitySpec[]): Promise<GivenEntity[]> => {
        return Promise.resolve(specs.filter((v, i) => i % 2 === 1).map((spec: GivenNewEntitySpec): GivenEntity => {
          return {
            ...spec,
            id: "DB_ID_" + spec.importId, // add the importId as the id so that we can find it later and check that it was mapped correctly
          };
