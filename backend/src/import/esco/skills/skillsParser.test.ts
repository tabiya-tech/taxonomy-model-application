...
describe("test parseSkills from", () => {
  ...
  test.each([
    [
      "url file",
      "./src/import/esco/skills/_test_data_/given.csv",
    ...
    async (
      ...
      const givenMockRepository: ISkillRepository = {
        ...
        createMany: jest
          .fn()
          .mockImplementation((specs: INewSkillSpec[]): Promise<ISkill[]> => {
      ...
      jest
        .spyOn(getRepositoryRegistry(), "skill", "get")
      ...
      expectedResults.forEach(
        (expectedSpec: Omit<INewSkillSpec, "modelId">) => {
      ...
      expectedResults
        .filter((res: Omit<INewSkillSpec, "modelId">) =>
          isSpecified(res.importId)
        )
        .forEach(
          (expectedSpec: Omit<INewSkillSpec, "modelId">, index: number) => {
      ...
      expect(importLogger.logWarning).toHaveBeenNthCalledWith(
        1,
        "Failed to import Skill from row:1 with importId:"
      );
      expect(importLogger.logWarning).toHaveBeenNthCalledWith(
        2,
        "Failed to import Skill from row:2 with importId:"
      );
    }
  );
});
