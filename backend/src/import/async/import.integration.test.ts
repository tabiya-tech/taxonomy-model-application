...
import {parseESCOSkillGroups, parseISCOGroups} from "./import";
...
describe("Test Import sample CSV files with an in-memory mongodb", () => {
  ...
  test("should import the sample CSV files", async () => {
    try {
      ...
      const iscoGroupsCSV = fs.readFileSync("../samples/csv-samples/ESCO dataset - v1.1.1/ISCOGroups_en.csv");
      const iscoGroups = extractData(iscoGroupsCSV);
      await parseISCOGroups(modelInfo.id, iscoGroups);
      ...
      const skillGroupsCSV = fs.readFileSync("../samples/csv-samples/ESCO dataset - v1.1.1/skillGroups_en.csv");
      const skillGroups = extractData(skillGroupsCSV);
      await parseESCOSkillGroups(modelInfo.id, skillGroups);
      expect(consoleErrorSpy).not.toHaveBeenCalled();
      expect(consoleWarnSpy).not.toHaveBeenCalled();
    } catch (err) {
