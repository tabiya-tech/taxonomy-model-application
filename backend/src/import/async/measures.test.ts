...
jest.mock("esco/interestingMeasures/OccupationMeasuresService");
...
describe("Interesting Measures", () => {
  describe("processMeasures - calculateDegreeCentrality", () => {
    test("should call calculateDegreeCentrality on call", async () => {
      ...
      expect(OccupationMeasuresService.prototype.calculate).toHaveBeenCalledWith(givenModelId);
    });
    ...
    test("should throw an error if calculateDegreeCentrality throws an error", async () => {
      ...
      jest.spyOn(OccupationMeasuresService.prototype, "calculate").mockRejectedValue(givenError);
