...
describe("test for trigger ImportHandler", () => {
  ...
  test.each([
     ["some of the file paths", { [Import.Types.ImportFileTypes.ESCO_SKILL]:"path/to/file"}],
     ["max payload size", Object.values(Import.Types.ImportFileTypes).reduce((accumulated, current) => {
       // @ts-ignore
    ...
    const givenPayload: Import.Types.ImportRequest = {
      modelId: getMockId(2),
