...
async function postTriggerImport(event: APIGatewayProxyEvent) {
  ...
  let payload: Import.Types.ImportRequest;
  try {
    payload = JSON.parse(event.body as string);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  } catch (error: any) {
    return errorResponse(StatusCodes.BAD_REQUEST, APIError.Types.ErrorCodes.MALFORMED_BODY, "Payload is malformed, it should be a valid model json", error.message);
  }
