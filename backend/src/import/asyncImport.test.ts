...
describe("Test lambda_invokeAsyncImport()  ", () => {
  test("should return the ACCEPTED status", async () => {
    // GIVEN an Import
    const givenImport: Import.Types.ImportRequest = {
      filePaths: {
  ...
  test("should return the INTERNAL_SERVER_ERROR status if InvokeCommand throws an error", async () => {
    ...
    const givenImport: Import.Types.ImportRequest = {
      filePaths: {
        [Import.Types.ImportFileTypes.ESCO_SKILL_GROUP]: "path/toESCO_SKILL_GROUP.csv",
        [Import.Types.ImportFileTypes.ESCO_SKILL]: "path/to/ESCO_SKILL.csv",
      },
      modelId: "foo"
    }
    ...
    const expectedErrorBody: APIError.Types.IErrorResponse = {
      "errorCode": Import.Types.ImportResponseErrorCodes.FAILED_TO_TRIGGER_IMPORT,
