...
export function parseAndValidatePOSTRequest(
...
): SkillAPISpecs.Skill.Parents.POST.Schemas.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: SkillAPISpecs.Skill.Parents.POST.Schemas.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    SkillAPISpecs.Skill.Parents.POST.Schemas.Request.Payload.$id as string
  ) as ValidateFunction;
