...
export function parseAndValidatePATCHRequest(
...
): SkillAPISpecs.Skill.Parents.PATCH.Types.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: SkillAPISpecs.Skill.Parents.PATCH.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    SkillAPISpecs.Skill.Parents.PATCH.Schemas.Request.Payload.$id as string
  ) as ValidateFunction | undefined;

  if (!validateFunction) {
    console.error("AJV schema not found for PATCH Skill Parents request payload");
