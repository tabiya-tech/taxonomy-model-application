...
export class SkillParentsPATCHController {
  /**
   * @openapi
   *
   * /models/{modelId}/skills/{id}/parents:
   *   patch:
   *    operationId: PATCHSkillParentsById
   *    tags:
   *      - skills
   *    summary: Update a skill parent relation.
   *    description: Update the parent relationship for a specific skill in a specific taxonomy model.
   *    security:
   *      - api_key: []
   *      - jwt_auth: []
   *    parameters:
   ...
   *         application/json:
   *           schema:
   *              $ref: '#/components/schemas/SkillParentsRequestSchemaPATCH'
   *       required: true
   *    responses:
   *      '200':
   *        description: Successfully updated the skill parent relation.
   *        content:
   *          application/json:
   *            schema:
   *               $ref: '#/components/schemas/SkillParentsResponseSchemaPATCH'
   *      '400':
   *        description: |
   *          Failed to update the parent relation. Additional information can be found in the response body.
   *        content:
   *          application/json:
   *            schema:
   *              $ref: '#/components/schemas/PATCHSkillParents400ErrorSchema'
   *      '401':
   ...
   *          application/json:
   *            schema:
   *              $ref: '#/components/schemas/PATCHSkillParents404ErrorSchema'
   *      '500':
   *        description: |
    ...
    try {
      ...
      if (validationResult === ModelForSkillValidationErrorCode.MODEL_NOT_FOUND_BY_ID) {
        return errorResponse(
          ...
          SkillAPISpecs.Skill.Parents.PATCH.Errors.Status404.ErrorCodes.MODEL_NOT_FOUND,
          "Model not found",
      ...
      if (validationResult === ModelForSkillValidationErrorCode.FAILED_TO_FETCH_FROM_DB) {
        return errorResponse(
          ...
          SkillAPISpecs.Skill.Parents.PATCH.Errors.Status500.ErrorCodes.DB_FAILED_TO_UPDATE_SKILL_PARENT_RELATION,
      ...
      if (validationResult === ModelForSkillValidationErrorCode.MODEL_IS_RELEASED) {
        return errorResponse(
          ...
          SkillAPISpecs.Skill.Parents.PATCH.Errors.Status400.ErrorCodes.MODEL_IS_RELEASED,
      ...
      const updatedParent = await skillHierarchyService.updateParent(
        params.modelId,
        params.id,
        ObjectTypes.Skill,
        payload.parentId,
      ...
      return responseJSON(StatusCodes.OK, buildParentResponse(updatedParent, getResourcesBaseUrl()));
    } catch (error: unknown) {
      console.error("Failed to update skill parent relation:", error);
      ...
      if (error instanceof SkillParentValidationError) {
        switch (error.code) {
          case ParentForSkillValidationErrorCode.SKILL_NOT_FOUND:
            return errorResponse(
              StatusCodes.NOT_FOUND,
              SkillAPISpecs.Skill.Parents.PATCH.Errors.Status404.ErrorCodes.SKILL_NOT_FOUND,
          ...
          case ParentForSkillValidationErrorCode.PARENT_NOT_FOUND:
            return errorResponse(
              StatusCodes.NOT_FOUND,
              SkillAPISpecs.Skill.Parents.PATCH.Errors.Status404.ErrorCodes.PARENT_NOT_FOUND,
              "Parent not found",
              ""
            );
          case ParentForSkillValidationErrorCode.PARENT_CHILD_CODE_INCONSISTENT:
            return errorResponse(
              StatusCodes.BAD_REQUEST,
              SkillAPISpecs.Skill.Parents.PATCH.Errors.Status400.ErrorCodes.PARENT_CHILD_CODE_INCONSISTENT,
              "Parent-Child code inconsistency",
              "Child code does not match parent code."
            );
          case ParentForSkillValidationErrorCode.DB_FAILED_TO_UPDATE_SKILL_PARENT_RELATION:
            return errorResponse(
              StatusCodes.INTERNAL_SERVER_ERROR,
              SkillAPISpecs.Skill.Parents.PATCH.Errors.Status500.ErrorCodes.DB_FAILED_TO_UPDATE_SKILL_PARENT_RELATION,
              "Failed to update skill parent relation in the DB",
              ""
            );
        }
      }
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  return new SkillParentsPATCHController().patch(event);
};
