...
import { handler as getParentsHandler } from "./index";
...
describe("SkillParentsGetController", () => {
  ...
  test("should return 200 and parents", async () => {
    const parent = getISkillMockData(3, givenModelId);
    ...
    const actualResponse = await getParentsHandler(event as unknown as APIGatewayProxyEvent);
  ...
  test("should return 400 if path params are invalid", async () => {
    const event = { path: "/models/invalid-uuid/skills/invalid-uuid/parents" };
    const actualResponse = await getParentsHandler(event as unknown as APIGatewayProxyEvent);
    expect(actualResponse.statusCode).toEqual(StatusCodes.BAD_REQUEST);
  });
  ...
  test("should return 500 if service fails", async () => {
    const givenSkillServiceMock = {
      validateModelForSkill: jest.fn().mockResolvedValue(null),
      findById: jest.fn().mockResolvedValue({ id: givenSkillId }),
      getParents: jest.fn().mockRejectedValue(new Error("DB error")),
    } as unknown as ISkillService;
    mockGetServiceRegistry.mockReturnValue({ skill: givenSkillServiceMock } as unknown as ServiceRegistry);
