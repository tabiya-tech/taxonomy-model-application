...
import { parseParentsGETQuery } from "./query";
import { buildParentsGETResponse } from "./response";

export class SkillParentsGetController {
  /**
   * @openapi
   *
  ...
  async get(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
      ...
      const paginationParams = parseParentsGETQuery(event);
      ...
      return responseJSON(
        StatusCodes.OK,
        buildParentsGETResponse(result.items, getResourcesBaseUrl(), paginationParams.limit, nextCursor)
      );
    } catch (error: unknown) {
      return errorResponseGET(
        StatusCodes.INTERNAL_SERVER_ERROR,
        SkillAPISpecs.GET.Errors.Status500.Parents.ErrorCodes.DB_FAILED_TO_RETRIEVE_SKILL_PARENTS,
        "Failed to retrieve skill parents from the DB",
        ""
      );
    }
  }
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  return new SkillParentsGetController().get(event);
};
