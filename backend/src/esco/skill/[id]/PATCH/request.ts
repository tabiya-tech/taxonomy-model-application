...
export function parseAndValidatePATCHRequest(
  ...
  if (event.body.length > SkillAPISpecs.Skill.PATCH.Constants.MAX_PATCH_PAYLOAD_LENGTH) {
    return STD_ERRORS_RESPONSES.TOO_LARGE_PAYLOAD_ERROR(
      `Expected maximum length is ${SkillAPISpecs.Skill.PATCH.Constants.MAX_PATCH_PAYLOAD_LENGTH}`
    );
  }

  let payload: SkillAPISpecs.Skill.PATCH.Types.Request.Payload;
  try {
