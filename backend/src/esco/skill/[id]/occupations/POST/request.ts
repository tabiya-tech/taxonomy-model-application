...
export function parseAndValidatePOSTRequest(
...
): SkillAPISpecs.Skill.Occupations.POST.Schemas.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: SkillAPISpecs.Skill.Occupations.POST.Schemas.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    SkillAPISpecs.Skill.Occupations.POST.Schemas.Request.Payload.$id as string
  ) as ValidateFunction;
