...
export function parseAndValidatePATCHRequest(
...
): SkillAPISpecs.Skill.Occupations.PATCH.Types.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: SkillAPISpecs.Skill.Occupations.PATCH.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    SkillAPISpecs.Skill.Occupations.PATCH.Schemas.Request.Payload.$id as string
  ) as ValidateFunction | undefined;

  if (!validateFunction) {
    console.error("AJV schema not found for PATCH Skill Occupations request payload");
