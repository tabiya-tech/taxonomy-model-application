...
import { handler as getRelatedHandler } from "./index";
...
describe("SkillRelatedGetController", () => {
  ...
  test("should return 200 and related skills", async () => {
    ...
    const event = { path: `/models/${givenModelId}/skills/${givenSkillId}/related` };
    const actualResponse = await getRelatedHandler(event as unknown as APIGatewayProxyEvent);
  ...
  test("should return 500 if service fails", async () => {
    const givenSkillServiceMock = {
      validateModelForSkill: jest.fn().mockResolvedValue(null),
      findById: jest.fn().mockResolvedValue({ id: givenSkillId }),
      getRelatedSkills: jest.fn().mockRejectedValue(new Error("DB error")),
    } as unknown as ISkillService;
    mockGetServiceRegistry.mockReturnValue({ skill: givenSkillServiceMock } as unknown as ServiceRegistry);
    const event = { path: `/models/${givenModelId}/skills/${givenSkillId}/related` };
    const actualResponse = await getRelatedHandler(event as unknown as APIGatewayProxyEvent);
  ...
  test("should return 200 with nextCursor when cursor is present", async () => {
    const givenSkillServiceMock = {
      validateModelForSkill: jest.fn().mockResolvedValue(null),
      findById: jest.fn().mockResolvedValue({ id: givenSkillId }),
      getRelatedSkills: jest.fn().mockResolvedValue({
        items: [],
