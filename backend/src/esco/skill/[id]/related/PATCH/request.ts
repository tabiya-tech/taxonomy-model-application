...
export function parseAndValidatePATCHRequest(
...
): SkillAPISpecs.Skill.RelatedSkills.PATCH.Types.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: SkillAPISpecs.Skill.RelatedSkills.PATCH.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    SkillAPISpecs.Skill.RelatedSkills.PATCH.Schemas.Request.Payload.$id as string
  ) as ValidateFunction | undefined;

  if (!validateFunction) {
    console.error("AJV schema not found for PATCH Skill Related request payload");
