...
export interface ISkillRepository {
   ...
   *
   * @param {INewSkillSpecWithoutImportId} newSkillSpec - The specification for the new Skill entry.
   * @return {Promise<ISkill>} - A Promise that resolves to the newly created Skill entry.
   * Rejects with an error if the Skill entry cannot be created due to reasons other than validation.
   */
  create(newSkillSpec: INewSkillSpecWithoutImportId): Promise<ISkill>;
...
export class SkillRepository implements ISkillRepository {
  ...
  async create(newSkillSpec: INewSkillSpecWithoutImportId): Promise<ISkill> {
    ...
    if (newSkillSpec.UUID !== undefined) {
      const err = new Error("SkillRepository.create: create failed. UUID should not be provided");
      console.error(err);
      throw err;
    }

    try {
      const newSkillModel = this.newSpecWithoutImportIdToModel(newSkillSpec);
