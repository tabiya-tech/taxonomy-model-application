...
export function parseAndValidatePOSTRequest(
...
): SkillAPISpecs.POST.Types.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: SkillAPISpecs.POST.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    SkillAPISpecs.POST.Schemas.Request.Payload.$id as string
  ) as ValidateFunction;
