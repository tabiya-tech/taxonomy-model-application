...
export function parseAndValidatePUTRequest(
...
): SkillGroupAPISpecs.SkillGroup.PUT.Types.Request.Payload | APIGatewayProxyResult {
  ...
  if (event.body.length > SkillGroupAPISpecs.SkillGroup.PUT.Constants.MAX_PUT_PAYLOAD_LENGTH) {
    return STD_ERRORS_RESPONSES.TOO_LARGE_PAYLOAD_ERROR(
      `Expected maximum length is ${SkillGroupAPISpecs.SkillGroup.PUT.Constants.MAX_PUT_PAYLOAD_LENGTH}`
    );
  }

  let payload: SkillGroupAPISpecs.SkillGroup.PUT.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    SkillGroupAPISpecs.SkillGroup.PUT.Schemas.Request.Payload.$id as string
  ) as ValidateFunction | undefined;

  if (!validateFunction) {
    console.error("AJV schema not found for PUT SkillGroup request payload");
