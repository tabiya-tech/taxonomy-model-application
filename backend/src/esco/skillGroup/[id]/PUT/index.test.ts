...
describe("SkillGroupPUTController", () => {
  ...
  test("should update a skill group and return the transformed response", async () => {
    ...
    // AND the service returns an updated skill group
    const updatedSkillGroup = { id: "group-1", UUID: "uuid-1" };
    const transformed = { id: "group-1", updated: true };
    mockBuildPUTResponse.mockReturnValue(transformed as never);

    const mockServiceRegistry = mockGetServiceRegistry();
    mockServiceRegistry.skillGroup.update = jest.fn().mockResolvedValue(updatedSkillGroup);
    ...
    expect(mockServiceRegistry.skillGroup.update).toHaveBeenCalledWith(
      "group-1",
      "model-1",
    ...
    // AND expect the response to be built from the updated skill group
    expect(mockBuildPUTResponse).toHaveBeenCalledWith(updatedSkillGroup, "https://resources.example.com");
  ...
  test("should respond with NOT_FOUND if the skill group does not exist", async () => {
    ...
    const mockServiceRegistry = mockGetServiceRegistry();
    mockServiceRegistry.skillGroup.update = jest.fn().mockResolvedValue(null);
    ...
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode: SkillGroupAPISpecs.SkillGroup.PUT.Errors.Response.Status404.ErrorCodes.SKILL_GROUP_NOT_FOUND,
  ...
  test("should respond with BAD_REQUEST if modelId in payload does not match modelId in path", async () => {
    ...
    const payload = { ...getValidPayload(), modelId: "model-2" };
    ...
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode: SkillGroupAPISpecs.SkillGroup.PUT.Errors.Response.Status400.ErrorCodes.INVALID_MODEL_ID,
  ...
  test("should respond with NOT_FOUND when the model does not exist", async () => {
    ...
    expect(actualResponse.statusCode).toBe(StatusCodes.NOT_FOUND);
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode: SkillGroupAPISpecs.SkillGroup.PUT.Errors.Response.Status404.ErrorCodes.MODEL_NOT_FOUND,
  ...
  test("should respond with BAD_REQUEST when the model is released", async () => {
    ...
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode: SkillGroupAPISpecs.SkillGroup.PUT.Errors.Response.Status400.ErrorCodes.UNABLE_TO_ALTER_RELEASED_MODEL,
  ...
  test("should respond with INTERNAL_SERVER_ERROR when failed to fetch model from DB", async () => {
    ...
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode: SkillGroupAPISpecs.SkillGroup.PUT.Errors.Response.Status500.ErrorCodes.DB_FAILED_TO_UPDATE_SKILL_GROUP,
  ...
  test("should respond with INTERNAL_SERVER_ERROR when service.update throws a generic error", async () => {
    ...
    const mockServiceRegistry = mockGetServiceRegistry();
    mockServiceRegistry.skillGroup.update = jest.fn().mockRejectedValue(new Error("Generic error"));
  ...
  test("should accept lowercase content-type header in request", async () => {
    ...
    const payload = getValidPayload();
    const updatedSkillGroup = { id: "group-1" };
    mockBuildPUTResponse.mockReturnValue({ id: "group-1" } as never);
    const mockServiceRegistry = mockGetServiceRegistry();
    mockServiceRegistry.skillGroup.update = jest.fn().mockResolvedValue(updatedSkillGroup);
  ...
  testTooLargePayload(HTTP_VERBS.PUT, SkillGroupAPISpecs.SkillGroup.PUT.Constants.MAX_PUT_PAYLOAD_LENGTH, putHandler);
});
