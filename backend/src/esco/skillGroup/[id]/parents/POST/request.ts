...
export function parseAndValidatePOSTRequest(
...
): SkillGroupAPISpecs.SkillGroup.Parent.POST.Types.Request.Payload | APIGatewayProxyResult {
  ...
  const maxPayloadLength = SkillGroupParentsPOSTAPISpecs.Constants.MAX_POST_PAYLOAD_LENGTH;
  ...
  let payload: SkillGroupAPISpecs.SkillGroup.Parent.POST.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    SkillGroupAPISpecs.SkillGroup.Parent.POST.Schemas.Request.Payload.$id as string
  ) as ValidateFunction;
