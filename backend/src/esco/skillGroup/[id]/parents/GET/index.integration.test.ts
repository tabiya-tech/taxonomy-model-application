...
describe("Test for skillGroup parents handler with a DB", () => {
  const ajv = new Ajv({
    validateSchema: true,
    strict: true,
    allErrors: true,
  ...
  addFormats(ajv);
  ajv.addSchema(SkillGroupAPISpecs.SkillGroup.Parent.GET.Schemas.Response.Parents.Payload);
  const validateResponse: ValidateFunction = ajv.getSchema(
    SkillGroupAPISpecs.SkillGroup.Parent.GET.Schemas.Response.Parents.Payload.$id as string
  ) as ValidateFunction;

  let dbConnection: Connection | undefined;
  beforeAll(async () => {
    const config = getTestConfiguration("SkillGroupParentsHandlerTestDB");
  ...
  test("GET should respond with the OK status code and an empty parents payload for an existing group with no parents", async () => {
    ...
    const givenEvent = {
      ...
      path: `/models/${givenModelInfo.id.toString()}/skillGroups/${givenSkillGroup.id.toString()}/parents`,
    };
