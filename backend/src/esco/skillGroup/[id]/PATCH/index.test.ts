...
import { SkillGroupPATCHController, handler as exportedPatchHandler } from "./index";
...
describe("SkillGroupPATCHController", () => {
  ...
  function buildEvent(body: unknown, path = "/models/model-1/skillGroups/group-1"): APIGatewayProxyEvent {
    return {
  ...
  test("should respond with FORBIDDEN status code if a user is not a model manager", async () => {
    ...
    const controller = new SkillGroupPATCHController();
  ...
  test("should patch a skill group and return the transformed response", async () => {
    ...
    // AND the service returns a patched skill group
    const patchedSkillGroup = { id: "group-1", UUID: "uuid-1" };
    const transformed = { id: "group-1", patched: true };
    mockBuildPATCHResponse.mockReturnValue(transformed as never);

    const mockServiceRegistry = mockGetServiceRegistry();
    mockServiceRegistry.skillGroup.patch = jest.fn().mockResolvedValue(patchedSkillGroup);
    ...
    expect(mockServiceRegistry.skillGroup.patch).toHaveBeenCalledWith(
      "group-1",
      "model-1",
    ...
    // AND expect the response to be built from the patched skill group
    expect(mockBuildPATCHResponse).toHaveBeenCalledWith(patchedSkillGroup, "https://resources.example.com");
  ...
  test("should patch a skill group with modelId provided in payload", async () => {
    ...
    const patchedSkillGroup = { id: "group-1" };
  ...
  test("should respond with NOT_FOUND if the skill group does not exist", async () => {
    ...
    const mockServiceRegistry = mockGetServiceRegistry();
    mockServiceRegistry.skillGroup.patch = jest.fn().mockResolvedValue(null);
    ...
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode: SkillGroupAPISpecs.SkillGroup.PATCH.Errors.Response.Status404.ErrorCodes.SKILL_GROUP_NOT_FOUND,
  ...
  test("should respond with BAD_REQUEST if modelId in payload does not match modelId in path", async () => {
    ...
    const givenEvent = {
      ...
      headers: { "Content-Type": "application/json" },
      pathParameters: { modelId: "model-1", id: "group-1" },
      path: "/models/model-1/skillGroups/group-1",
      requestContext: usersRequestContext.REGISTED_USER,
    } as never;
    ...
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode: SkillGroupAPISpecs.SkillGroup.PATCH.Errors.Response.Status400.ErrorCodes.INVALID_MODEL_ID,
  ...
  test("should respond with NOT_FOUND when the model does not exist", async () => {
    ...
    const mockServiceRegistry = mockGetServiceRegistry();
    mockServiceRegistry.skillGroup.patch = jest
      .fn()
      .mockRejectedValue(
    ...
    expect(actualResponse.statusCode).toBe(StatusCodes.NOT_FOUND);
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode: SkillGroupAPISpecs.SkillGroup.PATCH.Errors.Response.Status404.ErrorCodes.MODEL_NOT_FOUND,
  ...
  test("should respond with BAD_REQUEST when the model is released", async () => {
    ...
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      ...
      message: "Cannot update skill groups in a released model",
      details: "",
    };
    expect(JSON.parse(actualResponse.body)).toEqual(expectedErrorBody);
  });
  ...
  test("should respond with INTERNAL_SERVER_ERROR for unknown validation error code", async () => {
    ...
    mockServiceRegistry.skillGroup.patch = jest
      .fn()
      .mockRejectedValue(new SkillGroupModelValidationError(999 as ModelForSkillGroupValidationErrorCode));
    ...
    const expectedErrorBody: ErrorAPISpecs.Types.Payload = {
      errorCode:
        SkillGroupAPISpecs.SkillGroup.PATCH.Errors.Response.Status500.ErrorCodes.DB_FAILED_TO_UPDATE_SKILL_GROUP,
      message: "Failed to update the skill group in the DB",
      details: "",
    };
    expect(JSON.parse(actualResponse.body)).toEqual(expectedErrorBody);
  });
  ...
  test("should respond with INTERNAL_SERVER_ERROR when service.patch throws a generic error", async () => {
    ...
    const mockServiceRegistry = mockGetServiceRegistry();
    mockServiceRegistry.skillGroup.patch = jest.fn().mockRejectedValue(new Error("Generic error"));
  ...
  test("should patch all optional fields: originUri, code, altLabels, UUIDHistory, scopeNote", async () => {
    const validateFunction = jest.fn().mockReturnValue(true);
    getMockGetSchema().mockReturnValue(validateFunction as never);
  ...
  testTooLargePayload(
    HTTP_VERBS.PATCH,
    SkillGroupAPISpecs.SkillGroup.PATCH.Constants.MAX_PATCH_PAYLOAD_LENGTH,
    patchHandler
  );
});
