...
describe("Test for skillGroup detail handler with a DB", () => {
  const ajv = new Ajv({
    validateSchema: true,
    strict: true,
    allErrors: true,
  ...
  addFormats(ajv);
  ajv.addSchema(SkillGroupAPISpecs.SkillGroup.GET.Schemas.Response.Payload);
  ...
  beforeAll(async () => {
    const config = getTestConfiguration("SkillGroupDetailHandlerTestDB");
  ...
  test("GET should respond with the OK status code and the response passes the JSON Schema validation", async () => {
    ...
    const givenEvent = {
      ...
      path: `/models/${givenModelInfo.id.toString()}/skillGroups/${givenSkillGroup.id.toString()}`,
    };
