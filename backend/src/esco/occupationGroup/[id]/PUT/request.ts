...
export function parseAndValidatePUTRequest(
...
): OccupationGroupAPISpecs.OccupationGroup.PUT.Types.Request.Payload | APIGatewayProxyResult {
  ...
  if (event.body.length > OccupationGroupAPISpecs.OccupationGroup.PUT.Constants.MAX_PUT_PAYLOAD_LENGTH) {
    return STD_ERRORS_RESPONSES.TOO_LARGE_PAYLOAD_ERROR(
      `Expected maximum length is ${OccupationGroupAPISpecs.OccupationGroup.PUT.Constants.MAX_PUT_PAYLOAD_LENGTH}`
    );
  }

  let payload: OccupationGroupAPISpecs.OccupationGroup.PUT.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    OccupationGroupAPISpecs.OccupationGroup.PUT.Schemas.Request.Payload.$id as string
  ) as ValidateFunction | undefined;

  if (!validateFunction) {
    console.error("AJV schema not found for PUT OccupationGroup request payload");
