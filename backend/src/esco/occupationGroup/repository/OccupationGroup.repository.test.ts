...
import { IOccupationGroupRepository } from "./OccupationGroup.repository";
...
describe("Test the OccupationGroup Repository with an in-memory mongodb", () => {
  ...
  describe("Test findById()", () => {
    ...
    describe("should return the OccupationGroup with its parent and children", () => {
      test("should return the ISCOGroup with its parent(ISCOGroup) and children occupations(ESCOOccupation, LocalOccupation)", async () => {
        ...
        const givenChild_1 = await repositoryRegistry.occupation.create(toOccupationCreateSpec(givenChildSpecs_1));
        ...
        const givenChild_2 = await repositoryRegistry.occupation.create(toOccupationCreateSpec(givenChildSpecs_2));
    ...
    describe.each<[string, ObjectTypes.ISCOGroup | ObjectTypes.LocalGroup]>([
      ...
      test("should not match entities that have the same ID but are of different types (collections) when populating children", async () => {
        ...
        const givenOccupation_1 = await repositoryRegistry.occupation.create(
          toOccupationCreateSpec(givenOccupation1Specs)
        );
        // guard to ensure the id is the given one
        ...
        const givenOccupation_3 = await repositoryRegistry.occupation.create(
          toOccupationCreateSpec(givenOccupationSpecs_3)
        );
  ...
  describe("Test findChildren()", () => {
    test("should find an OccupationGroup | Occupation children by parentId", async () => {
      ...
      const givenChild_4 = await repositoryRegistry.occupation.create(toOccupationCreateSpec(givenChildSpecs_4));
      // AND the subject OccupationGroup has a parent and two children
      const actualHierarchy = await repositoryRegistry.occupationHierarchy.createMany(givenModelId, [
        {
          // parent of the subject
  ...
  describe("Test findPaginated()", () => {
    ...
    describe("with a search value", () => {
      test("should only return the OccupationGroups whose requested field matches the search value (case-insensitive)", async () => {
        ...
        // THEN expect only the two matching occupation groups
        expect(actual.map((g) => g.id).sort()).toEqual([givenNursing.id, givenNursingAssoc.id].sort());
      });
      ...
      test("should treat the search value literally (regex special characters are escaped)", async () => {
        ...
        // THEN expect only the occupation group that matches literally
  ...
  describe("Test findByIds()", () => {
    test("should return the OccupationGroups of the model with the given ids", async () => {
      ...
      // THEN expect exactly those two occupation groups back (order not guaranteed)
    ...
    test("should ignore invalid ids and ids that do not belong to the model", async () => {
      ...
      // THEN expect only the occupation group of the model to be returned
      expect(actual).toHaveLength(1);
    ...
    test("should reject when the database query fails", async () => {
      ...
      await expect(repository.findByIds(getMockStringId(1), [getMockStringId(2)])).rejects.toThrow(
        new Error("OccupationGroupRepository.findByIds: findByIds failed", { cause: givenError })
      );
    });
  });
