...
function testObjectIdField<T>(getModel: () => mongoose.Model<T>, fieldName: string) {
  return describe(`Test validation of '${fieldName}'`, () => {
    ...
    ])(`(%s) Validate '${fieldName}' when it is %s`, (caseType: CaseType, caseDescription, value, expectedFailureMessage) => {
...
function testObjectType<T>(getModel: () => mongoose.Model<T>, fieldName: string) {
  describe(`Test validation of '${fieldName}'`, () => {
    test.each([
      ...
      [CaseType.Failure, ObjectTypes.Skill, ObjectTypes.Skill, `\`${ObjectTypes.Skill}\` is not a valid enum value for path \`{0}\`.`],
      [CaseType.Failure, ObjectTypes.SkillGroup, ObjectTypes.SkillGroup, `\`${ObjectTypes.SkillGroup}\` is not a valid enum value for path \`{0}\`.`],
