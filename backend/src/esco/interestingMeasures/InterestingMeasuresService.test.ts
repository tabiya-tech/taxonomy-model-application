...
import { InterestingMeasuresService, ISkillConnection } from "esco/interestingMeasures/InterestingMeasuresService";
...
describe("InterestingMeasuresService", () => {
  ...
  let service: InterestingMeasuresService;

  beforeAll(async () => {
    const config = getTestConfiguration("InterestingMeasuresService");
    ...
    service = new InterestingMeasuresService(
  ...
  describe("Test updateSkillDegreeCentrality() ", () => {
    ...
    test("should handle errors during data retrieval", async () => {
      ...
      await expect(actualOccupationToSkillRelations).rejects.toThrow(
        new Error("InterestingMeasuresService.updateSkillDegreeCentrality: bulkWrite failed", { cause: givenError })
      );
    });
  ...
  describe("Test aggregateDegreeCentralityData()", () => {
    ...
    test("should handle errors during data retrieval", async () => {
      ...
      expect(actualOccupationToSkillRelations).toThrow(
        expect.toMatchErrorWithCause(
          "InterestingMeasuresService.updateSkillDegreeCentrality: aggregate failed",
          givenError.message
        )
      );
    });
    ...
    test("should end and emit an error if an error occurs during data retrieval in the upstream", async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.toMatchErrorWithCause(
          "InterestingMeasuresService.updateSkillDegreeCentralit: stream failed",
          givenError.message
        )
      );
  ...
  describe("Test calculateDegreeCentrality", () => {
    const aggregateDegreeCentralityDataSpy = jest.spyOn(
      InterestingMeasuresService.prototype,
      "aggregateDegreeCentralityData"
    );
    const updateSkillDegreeCentralitySpy = jest.spyOn(InterestingMeasuresService.prototype, "updateSkillDegreeCentrality");
    ...
    it("should log any error if aggregateDegreeCentralityData throw an error", async () => {
      ...
      await expect(() => service.calculateDegreeCentrality(givenModelId)).rejects.toThrow(
        expect.toMatchErrorWithCause("InterestingMeasuresService.calculateDegreeCentrality findById failed", "foo")
      );
    });
  });
});
