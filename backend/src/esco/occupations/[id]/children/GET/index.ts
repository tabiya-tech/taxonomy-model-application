...
export class OccupationChildrenController {
   ...
   *      '400':
   *        description: |
   *          Failed to retrieve the occupation children. Additional information can be found in the response body.
   *        content:
   *          application/json:
   *            schema:
  ...
  async get(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    try {
      const params = extractAndValidateIdParams(event, Routes.OCCUPATION_CHILDREN_ROUTE);
      ...
      const currentPageChildren = await service.getChildren(
        params.modelId,
        params.id,
      ...
      if (currentPageChildren?.nextCursor?._id) {
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  return new OccupationChildrenController().get(event);
};
