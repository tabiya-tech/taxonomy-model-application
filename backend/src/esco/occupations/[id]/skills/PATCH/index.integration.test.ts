...
describe("Test for occupation Skills PATCH handler with a DB", () => {
  const ajv = new Ajv({
    validateSchema: true,
    strict: true,
    allErrors: true,
  ...
  addFormats(ajv);
  ajv.addSchema(OccupationAPISpecs.Occupation.Skills.PATCH.Schemas.Response.Payload);

  const validateSkillResponse: ValidateFunction = ajv.getSchema(
    OccupationAPISpecs.Occupation.Skills.PATCH.Schemas.Response.Payload.$id as string
  ) as ValidateFunction;

  let dbConnection: Connection | undefined;
  beforeAll(async () => {
    const config = getTestConfiguration("OccupationSkillsPATCHHandlerTestDB");
  ...
  test("PATCH should respond with OK status code and correctly update relation in DB", async () => {
