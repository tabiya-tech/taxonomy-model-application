...
export class OccupationSkillsPATCHController {
  /**
   * @openapi
   *
   * /models/{modelId}/occupations/{id}/skills:
   *   patch:
   *    operationId: PATCHOccupationSkillsById
   *    tags:
   *      - occupations
   *    summary: Update an occupation skill relation.
   *    description: Update relationship metadata between an occupation and a skill in a specific taxonomy model.
   *    security:
   *      - api_key: []
   *      - jwt_auth: []
   *    parameters:
   ...
   *         application/json:
   *           schema:
   *              $ref: '#/components/schemas/OccupationSkillsRequestSchemaPATCH'
   *       required: true
   *    responses:
   *      '200':
   *        description: Successfully updated the occupation skill relation.
   *        content:
   *          application/json:
   *            schema:
   *               $ref: '#/components/schemas/OccupationSkillItemResponseSchemaPATCH'
   *      '400':
   *        description: |
   *          Failed to update the occupation skill relation. Additional information can be found in the response body.
   *        content:
   *          application/json:
   *            schema:
    ...
    try {
      ...
      if (validationResult === ModelForOccupationValidationErrorCode.MODEL_NOT_FOUND_BY_ID) {
        return errorResponse(
          ...
          OccupationAPISpecs.Occupation.Skills.PATCH.Errors.Status404.ErrorCodes.MODEL_NOT_FOUND,
          "Model not found",
      ...
      if (validationResult === ModelForOccupationValidationErrorCode.MODEL_IS_RELEASED) {
        return errorResponse(
          ...
          OccupationAPISpecs.Occupation.Skills.PATCH.Errors.Status400.ErrorCodes.MODEL_IS_RELEASED,
      ...
      try {
        ...
        if (error instanceof OccupationSkillValidationError) {
          if (error.code === SkillForOccupationValidationErrorCode.OCCUPATION_NOT_FOUND) {
            return errorResponse(
              StatusCodes.NOT_FOUND,
              OccupationAPISpecs.Occupation.Skills.PATCH.Errors.Status404.ErrorCodes.OCCUPATION_NOT_FOUND,
          ...
          if (error.code === SkillForOccupationValidationErrorCode.SKILL_NOT_FOUND) {
            return errorResponse(
              StatusCodes.NOT_FOUND,
              OccupationAPISpecs.Occupation.Skills.PATCH.Errors.Status404.ErrorCodes.SKILL_NOT_FOUND,
          ...
          if (error.code === SkillForOccupationValidationErrorCode.INVALID_RELATION_TYPE) {
            return errorResponse(
              StatusCodes.BAD_REQUEST,
              OccupationAPISpecs.Occupation.Skills.PATCH.Errors.Status400.ErrorCodes.INVALID_RELATION_TYPE,
          ...
          if (error.code === SkillForOccupationValidationErrorCode.RELATION_CODE_INCONSISTENT) {
            return errorResponse(
              StatusCodes.BAD_REQUEST,
              OccupationAPISpecs.Occupation.Skills.PATCH.Errors.Status400.ErrorCodes.RELATION_CODE_INCONSISTENT,
          ...
          if (error.code === SkillForOccupationValidationErrorCode.MUTUALLY_EXCLUSIVE_VALUES) {
            return errorResponse(
              StatusCodes.BAD_REQUEST,
              OccupationAPISpecs.Occupation.Skills.PATCH.Errors.Status400.ErrorCodes.MUTUALLY_EXCLUSIVE_VALUES,
      ...
      return responseJSON(StatusCodes.OK, buildSkillsResponse(transformedSkill, getResourcesBaseUrl()));
    } catch (error: unknown) {
      errorLoggerInstance.logError(
        "Failed to update occupation skill relation in the DB",
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  return new OccupationSkillsPATCHController().patch(event);
};
