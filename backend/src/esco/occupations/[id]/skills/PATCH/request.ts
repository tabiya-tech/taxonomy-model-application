...
export function parseAndValidatePATCHRequest(
...
): OccupationAPISpecs.Occupation.Skills.PATCH.Types.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: OccupationAPISpecs.Occupation.Skills.PATCH.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    OccupationAPISpecs.Occupation.Skills.PATCH.Schemas.Request.Payload.$id as string
  ) as ValidateFunction | undefined;

  if (!validateFunction) {
    console.error("AJV schema not found for PATCH Occupation Skills request payload");
