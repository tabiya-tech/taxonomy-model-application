...
export function parseAndValidatePOSTRequest(
...
): OccupationAPISpecs.Occupation.Skills.POST.Schemas.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: OccupationAPISpecs.Occupation.Skills.POST.Schemas.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    OccupationAPISpecs.Occupation.Skills.POST.Schemas.Request.Payload.$id as string
  ) as ValidateFunction;
