...
describe("Test for occupation Parent PATCH handler with a DB", () => {
  ...
  ajv.addSchema(OccupationAPISpecs.Occupation.Parent.PATCH.Schemas.Response.Payload);

  const validateParentResponse: ValidateFunction = ajv.getSchema(
    OccupationAPISpecs.Occupation.Parent.PATCH.Schemas.Response.Payload.$id as string
  ) as ValidateFunction;

  let dbConnection: Connection | undefined;
  beforeAll(async () => {
    const config = getTestConfiguration("OccupationParentPATCHHandlerTestDB");
  ...
  test("PATCH should respond with FORBIDDEN status code if the user is not a model manager", async () => {
  ...
  test("PATCH should respond with OK status code and update child-parent link in DB", async () => {
