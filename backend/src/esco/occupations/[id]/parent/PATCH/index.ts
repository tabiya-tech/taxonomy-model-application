...
export class OccupationParentPATCHController {
  /**
   * @openapi
   *
   * /models/{modelId}/occupations/{id}/parent:
   *   patch:
   *    operationId: PATCHOccupationParentById
   *    tags:
   *      - occupations
   *    summary: Update an occupation parent.
   ...
   *         application/json:
   *           schema:
   *              $ref: '#/components/schemas/OccupationParentRequestSchemaPATCH'
   *       required: true
   *    responses:
   *      '200':
   *        description: Successfully updated the occupation parent.
   *        content:
   *          application/json:
   *            schema:
    ...
    try {
      ...
      if (validationResult === ModelForOccupationValidationErrorCode.MODEL_NOT_FOUND_BY_ID) {
        return errorResponse(
          ...
          OccupationAPISpecs.Occupation.Parent.PATCH.Errors.Status404.ErrorCodes.MODEL_NOT_FOUND,
          "Model not found",
      ...
      if (validationResult === ModelForOccupationValidationErrorCode.MODEL_IS_RELEASED) {
        return errorResponse(
          ...
          OccupationAPISpecs.Occupation.Parent.PATCH.Errors.Status400.ErrorCodes.MODEL_IS_RELEASED,
      ...
      try {
        ...
        if (error instanceof OccupationParentValidationError) {
          if (error.code === ParentForOccupationValidationErrorCode.OCCUPATION_NOT_FOUND) {
            return errorResponse(
              StatusCodes.NOT_FOUND,
              OccupationAPISpecs.Occupation.Parent.PATCH.Errors.Status404.ErrorCodes.OCCUPATION_NOT_FOUND,
          ...
          if (error.code === ParentForOccupationValidationErrorCode.PARENT_NOT_FOUND) {
            return errorResponse(
              StatusCodes.NOT_FOUND,
              OccupationAPISpecs.Occupation.Parent.PATCH.Errors.Status404.ErrorCodes.PARENT_NOT_FOUND,
              "Parent not found",
          ...
          if (error.code === ParentForOccupationValidationErrorCode.INVALID_PARENT_TYPE) {
            return errorResponse(
              StatusCodes.BAD_REQUEST,
              OccupationAPISpecs.Occupation.Parent.PATCH.Errors.Status400.ErrorCodes.INVALID_PARENT_TYPE,
      ...
      return responseJSON(StatusCodes.OK, buildParentResponse(transformedParent, getResourcesBaseUrl()));
    } catch (error: unknown) {
      errorLoggerInstance.logError(
        "Failed to update occupation parent in the DB",
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  return new OccupationParentPATCHController().patch(event);
};
