...
export function parseAndValidatePATCHRequest(
...
): OccupationAPISpecs.Occupation.Parent.PATCH.Types.Request.Payload | APIGatewayProxyResult {
  ...
  let payload: OccupationAPISpecs.Occupation.Parent.PATCH.Types.Request.Payload;
  try {
  ...
  const validateFunction = ajvInstance.getSchema(
    OccupationAPISpecs.Occupation.Parent.PATCH.Schemas.Request.Payload.$id as string
  ) as ValidateFunction | undefined;

  if (!validateFunction) {
    console.error("AJV schema not found for PATCH Occupation Parent request payload");
