...
function getNewOccupationSpecLocalized(overrides?: Partial<INewOccupationSpecLocalized>): INewOccupationSpecLocalized {
  ...
  return {
    ...
    preferredLabel: new Map([[fallbackKey, "preferred label"]]) as INewOccupationSpecLocalized["preferredLabel"],
    altLabels: [
      new Map([[fallbackKey, "alt label 1"]]),
      new Map([[fallbackKey, "alt label 2"]]),
    ] as INewOccupationSpecLocalized["altLabels"],
    description: new Map([[fallbackKey, "description"]]) as INewOccupationSpecLocalized["description"],
    definition: new Map([[fallbackKey, "definition"]]) as INewOccupationSpecLocalized["definition"],
    scopeNote: new Map([[fallbackKey, "scope note"]]) as INewOccupationSpecLocalized["scopeNote"],
    regulatedProfessionNote: new Map([
      [fallbackKey, "regulated profession note"],
    ]) as INewOccupationSpecLocalized["regulatedProfessionNote"],
    ...overrides,
  };
}

function expectedFromGivenLocalizedSpec(givenSpec: INewOccupationSpecLocalized, newUUID: string): IOccupation {
  const fallbackKey = getFallbackLanguageConfig().dbKeyName as Parameters<
    INewOccupationSpecLocalized["preferredLabel"]["get"]
  >[0];
  return {
    ...givenSpec,
    parent: null,
    ...
    regulatedProfessionNote: givenSpec.regulatedProfessionNote.get(fallbackKey) ?? "",
    createdAt: expect.any(Date),
    updatedAt: expect.any(Date),
  };
}
...
describe("Test the Occupation Repository with an in-memory mongodb", () => {
  ...
  describe("Test createManyLocalized() Occupation", () => {
    test("should successfully create a batch of new occupations from localized specs", async () => {
      // GIVEN some valid localized OccupationSpecs
      const givenBatchSize = 3;
      const givenSpecs: INewOccupationSpecLocalized[] = [];
      for (let i = 0; i < givenBatchSize; i++) {
        givenSpecs[i] = getNewOccupationSpecLocalized();
      }

      // WHEN creating the batch
      const actualNewOccupations: IOccupation[] = await repository.createManyLocalized(givenSpecs);

      // THEN expect all Occupations to be created with the specific attributes
      expect(actualNewOccupations).toEqual(
        expect.arrayContaining(
          givenSpecs.map((spec, index) => expectedFromGivenLocalizedSpec(spec, actualNewOccupations[index].UUID))
        )
      );
    });

    test("should successfully create a batch even if some don't validate", async () => {
      // GIVEN two valid localized OccupationSpecs
      const givenValidSpecs: INewOccupationSpecLocalized[] = [
        getNewOccupationSpecLocalized(),
        getNewOccupationSpecLocalized(),
      ];
      // AND one invalid spec (unknown extra field)
      const givenInvalidSpec = getNewOccupationSpecLocalized();
      // @ts-ignore
      givenInvalidSpec.foo = "invalid";

      // WHEN creating with a mix of valid and invalid
      const actualNewOccupations: IOccupation[] = await repository.createManyLocalized([
        givenValidSpecs[0],
        givenInvalidSpec,
        givenValidSpecs[1],
      ]);
      ...
      // THEN expect only the valid Occupations to be created
      expect(actualNewOccupations).toHaveLength(givenValidSpecs.length);
      expect(actualNewOccupations).toEqual(
        expect.arrayContaining(
          givenValidSpecs.map((spec, index) => expectedFromGivenLocalizedSpec(spec, actualNewOccupations[index].UUID))
        )
      );
    });

    test.each([0, 1, 2, 10])(
      "should successfully create a batch of localized occupations when they have UUIDHistory with %i UUIDs",
      async (count: number) => {
        // GIVEN some valid localized OccupationSpecs with varying UUIDHistory lengths
        const givenBatchSize = 3;
        const givenSpecs: INewOccupationSpecLocalized[] = [];
        for (let i = 0; i < givenBatchSize; i++) {
          givenSpecs[i] = getNewOccupationSpecLocalized();
    ...
    test("should resolve to an empty array if none of the elements could be validated", async () => {
      // GIVEN only invalid localized OccupationSpecs
      const givenBatchSize = 3;
      const givenInvalidSpecs: INewOccupationSpecLocalized[] = [];
      for (let i = 0; i < givenBatchSize; i++) {
        givenInvalidSpecs[i] = getNewOccupationSpecLocalized();
        // @ts-ignore
        givenInvalidSpecs[i].foo = "invalid";
      }

      // WHEN creating the batch
      const actualNewOccupations: IOccupation[] = await repository.createManyLocalized(givenInvalidSpecs);

      // THEN expect no Occupations to be created
      expect(actualNewOccupations).toHaveLength(0);
    });
    ...
    test("should store the localized Maps directly in the database under the correct language keys", async () => {
      ...
      const fallbackKey = getFallbackLanguageConfig().dbKeyName;
      const givenSpec = getNewOccupationSpecLocalized({
        preferredLabel: new Map([[fallbackKey, "my preferred label"]]) as INewOccupationSpecLocalized["preferredLabel"],
        description: new Map([[fallbackKey, "my description"]]) as INewOccupationSpecLocalized["description"],
        definition: new Map([[fallbackKey, "my definition"]]) as INewOccupationSpecLocalized["definition"],
        scopeNote: new Map([[fallbackKey, "my scope note"]]) as INewOccupationSpecLocalized["scopeNote"],
        regulatedProfessionNote: new Map([
          [fallbackKey, "my regulated profession note"],
        ]) as INewOccupationSpecLocalized["regulatedProfessionNote"],
        altLabels: [new Map([[fallbackKey, "my alt label"]])] as INewOccupationSpecLocalized["altLabels"],
      });
      ...
      expect(actualRawDoc?.regulatedProfessionNote).toEqual({ [fallbackKey]: "my regulated profession note" });
      ...
      expect(actualCreated.regulatedProfessionNote).toEqual("my regulated profession note");
    ...
    describe("Test unique indexes", () => {
      test("should return only documents that did not violate the UUID unique index", async () => {
        // GIVEN 3 localized OccupationSpecs
        const givenBatchSize = 3;
        const givenSpecs: INewOccupationSpecLocalized[] = [];
        for (let i = 0; i < givenBatchSize; i++) {
          givenSpecs[i] = getNewOccupationSpecLocalized();
        ...
        // THEN expect only the first and third Occupations to be created
        expect(actualNewOccupations).toEqual(
          expect.arrayContaining(
            givenSpecs
              .filter((_, index) => index !== 1)
              .map((spec, index) => expectedFromGivenLocalizedSpec(spec, actualNewOccupations[index].UUID))
          )
        );
      });
    });
    ...
    TestDBConnectionFailureNoSetup<unknown>((repositoryRegistry) => {
      return repositoryRegistry.occupation.createManyLocalized([getNewOccupationSpecLocalized()]);
    });
  });
  ...
  describe("Test findPaginated()", () => {
    test("should return first page when cursor is undefined", async () => {
      ...
      const actualSecondPage = await repository.findPaginated(givenModelId, 2, -1, actualFirstPage[1].id);

      // THEN the second page should contain the remaining occupation
      expect(actualSecondPage).toHaveLength(1);
      expect(actualSecondPage[0].id).toEqual(givenOccupations[0].id);
    });
    ...
    test("should return paginated Occupations for a given modelId, limit and cursor", async () => {
      ...
      const actualFirstPageOccupationsArray = await repository.findPaginated(givenModelId, 2, -1);
    ...
    test("should reject when database query fails", async () => {
      ...
      } as unknown as mongoose.Aggregate<IOccupationDoc[]>);
    ...
    test("should paginate consistently with ascending sort", async () => {
      ...
      // THEN expect to get first two items in ascending order by _id
      expect(page1).toHaveLength(2);
      expect(page1.map((i) => i.id)).toEqual([givenOccupations[0].id, givenOccupations[1].id]);
      ...
      expect(page2[0].id).toEqual(givenOccupations[2].id);
    });
    ...
    test("should ignore invalid cursor ID", async () => {
      ...
      expect(actual[0].id).toBe(givenOccupation.id);
    });
    ...
    describe("with a search value", () => {
      test("should only return the Occupations whose requested field matches the search value (case-insensitive)", async () => {
        ...
        // THEN expect only the two matching occupations
        expect(actual.map((o) => o.id).sort()).toEqual([givenSoftwareEngineer.id, givenSoftwareArchitect.id].sort());
      });
      ...
      test("should treat the search value literally (regex special characters are escaped)", async () => {
        ...
        // THEN expect only the occupation that matches literally (not the wildcard interpretation)
        expect(actual).toHaveLength(1);
        expect(actual[0].id).toBe(givenOccupation.id);
      });
  ...
  describe("Test findByIds()", () => {
    test("should return the Occupations of the model with the given ids", async () => {
      ...
      // THEN expect exactly those two occupations back (order not guaranteed)
      expect(actual.map((o) => o.id).sort()).toEqual([given1.id, given2.id].sort());
    });
    ...
    test("should ignore invalid ids and ids that do not belong to the model", async () => {
      ...
      // THEN expect only the occupation of the model to be returned
      expect(actual).toHaveLength(1);
    ...
    test("should return an empty array when no valid ids are given", async () => {
      ...
      // THEN expect an empty array (and no query is executed)
    ...
    test("should reject when the database query fails", async () => {
      ...
      await expect(repository.findByIds(getMockStringId(1), [getMockStringId(2)])).rejects.toThrow(
        new Error("OccupationRepository.findByIds: findByIds failed", { cause: givenError })
      );
  ...
  describe("Test findHistoryReferencesByUUIDs()", () => {
    test("should resolve each UUID to its occupation reference + modelId, preserving input order and null-filling misses", async () => {
      ...
      const actual = await repository.findHistoryReferencesByUUIDs([
        givenOccupation1.UUID,
        givenMissingUUID,
        givenOccupation2.UUID,
      ]);

      // THEN expect one entry per input UUID in input order, with the reference + modelId for matches and nulls for the miss
      expect(actual).toEqual([
        {
          UUID: givenOccupation1.UUID,
          modelId: givenModelId1,
          reference: {
            id: givenOccupation1.id,
            UUID: givenOccupation1.UUID,
            preferredLabel: givenOccupation1.preferredLabel,
            occupationGroupCode: givenOccupation1.occupationGroupCode,
            code: givenOccupation1.code,
            occupationType: givenOccupation1.occupationType,
            isLocalized: givenOccupation1.isLocalized,
          },
        },
        { UUID: givenMissingUUID, modelId: null, reference: null },
        {
          UUID: givenOccupation2.UUID,
          modelId: givenModelId2,
          reference: {
            id: givenOccupation2.id,
            UUID: givenOccupation2.UUID,
            preferredLabel: givenOccupation2.preferredLabel,
            occupationGroupCode: givenOccupation2.occupationGroupCode,
            code: givenOccupation2.code,
            occupationType: givenOccupation2.occupationType,
            isLocalized: givenOccupation2.isLocalized,
          },
        },
      ]);
    });
    ...
    TestDBConnectionFailureNoSetup<unknown>((repositoryRegistry) => {
      return repositoryRegistry.occupation.findHistoryReferencesByUUIDs([randomUUID()]);
    });
  });
    ...
    describe("test Occupation to Skill relations robustness to inconsistencies", () => {
      test("should ignore requiresSkills that are not Skills", async () => {
        ...
        const givenInconsistentPair: IOccupationToSkillRelationPairDoc = {
          ...
          requiredSkillId: new mongoose.Types.ObjectId(givenOccupationGroup.id),
          //@ts-ignore
          requiredSkillDocModel: MongooseModelName.OccupationGroup,
        };
  ...
  describe("Test findAllWithTranslations()", () => {
    // the flat Occupation the repository returns, wrapped back into the translations it is stored as
    function toTranslatedOccupation(entity: IOccupation): IOccupationWithTranslations {
      const { parent, children, requiresSkills, ...entityData } = entity;
      return {
        ...entityData,
        preferredLabel: wrapTranslated(entityData.preferredLabel),
        description: wrapTranslated(entityData.description),
        definition: wrapTranslated(entityData.definition),
        scopeNote: wrapTranslated(entityData.scopeNote),
        regulatedProfessionNote: wrapTranslated(entityData.regulatedProfessionNote),
        altLabels: wrapTranslatedArray(entityData.altLabels),
      };
    }

    test("should find all Occupations in the correct model, with every translation of their translatable fields", async () => {
      // Given some modelId
      const givenModelId = getMockStringId(1);
      // AND a set of Occupations exist in the database for the given Model
      const givenEntities = await createOccupationsInDB(givenModelId);
      // AND some other Occupations exist in the database for a different model
      await createOccupationsInDB(getMockStringId(2));
      // AND the first Occupation is also translated in French, stored directly (the flat-string repository API has no
      ...
      // WHEN searching for all Occupations with their translations in the given model
      const actualStream = repository.findAllWithTranslations(givenModelId);

      // THEN the Occupations should be returned as a consumable stream that emits all Occupations of the given model
      const actualEntities: IOccupationWithTranslations[] = [];
      for await (const data of actualStream) {
        actualEntities.push(data);
      }
      // AND their translatable fields should not be flattened to the fallback language
      const expectedEntities = givenEntities.map(toTranslatedOccupation);
      expectedEntities[0] = {
        ...expectedEntities[0],
        ...
        updatedAt: expect.any(Date),
      } as IOccupationWithTranslations;
      expect(actualEntities).toIncludeSameMembers(expectedEntities);
    });

    test("should not return any Occupations when the model does not have any and other models have", async () => {
      // GIVEN no Occupations exist in the database for the given model
      const givenModelId = getMockStringId(1);
      // BUT some other Occupations exist in the database for a different model
      await createOccupationsInDB(getMockStringId(2));
      ...
      // THEN the stream should end without emitting any data
      const receivedData: IOccupationWithTranslations[] = [];
      for await (const data of actualStream) {
        receivedData.push(data);
      }
      expect(receivedData).toHaveLength(0);
    ...
    test("should handle errors during data retrieval", async () => {
      ...
      expect(() => repository.findAllWithTranslations(givenModelId)).toThrow(
        expect.toMatchErrorWithCause(
          "OccupationRepository.findAllWithTranslations: findAllWithTranslations failed",
          givenError.message
        )
      );
    });
    ...
    TestStreamDBConnectionFailureNoSetup((repositoryRegistry) =>
      repositoryRegistry.occupation.findAllWithTranslations(getMockStringId(1))
    );
  });
  ...
  describe("Test findParent()", () => {
    ...
    test("should log a warning if multiple parents are found", async () => {
      ...
      // THEN expect the first parent to be returned (mocked hydrate is complex, so we just check it tried to hydrate)
      expect(result).not.toBeNull();
  ...
  describe("findSkillsForOccupation", () => {
    test("should return skills for an occupation", async () => {
      ...
      const skills = await repository.findSkillsForOccupation(modelId, occupation.id, 10);
      expect(skills).toHaveLength(1);
      expect(skills[0].preferredLabel).toEqual("skill");
    });
    ...
    test("should support pagination with cursor", async () => {
      ...
      // Sort by ID to match repository logic
      skillsInDB.sort((a, b) => a.id.toString().localeCompare(b.id.toString()));
      ...
      expect(page1).toHaveLength(2);
      expect(page1[0].id).toEqual(skillsInDB[0].id.toString());

      // Get second page using cursor
      const page2 = await repository.findSkillsForOccupation(modelId, occupation.id, 2, skillsInDB[1].id.toString());
      expect(page2).toHaveLength(1);
      expect(page2[0].id).toEqual(skillsInDB[2].id.toString());
    });
    ...
    test("should handle errors in findSkillsForOccupation", async () => {
      ...
      const RelationModel = dbConnection.model(MongooseModelName.OccupationToSkillRelation);
      const aggregateSpy = jest.spyOn(RelationModel, "aggregate");
      aggregateSpy.mockImplementationOnce(() => {
        throw new Error("Database error");
      });
